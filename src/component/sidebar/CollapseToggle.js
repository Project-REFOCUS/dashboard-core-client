...
const CollapseToggle = ({
  ...
  const toggleDuplicate = () => setIsDuplicate(type);
  ...
  return (
    <div className="d-flex justify-content-between align-items-center">
      ...
      {showCollapse && ctType === "second" && <TrashIcon />}
    </div>
  );
};

const PlusModes = ({ length, toggleDuplicate }) => {
  return (
    <>
      {length < 2 ? (
        <PlusLg
...
const TrashIcon = ({ handleClick }) => (
  <div className="trash-icon-svg" onClick={handleClick}>
    <svg
      width="14"
      height="14"
      viewBox="0 0 14 14"
