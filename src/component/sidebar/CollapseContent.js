...
const CollapseContent = ({
  ...
  type,
  handleContentChange,
  ctType = "first", // ctType is "second" -> collapseContent is from duplicate
}) => {
  ...
  const handleReactSelectChange = (selected, action) => {
    switch (action.name) {
      case "dataOrientationSelectName":
        ...
        handleContentChange(type, { dataOrientation: selected.value }, ctType);
        break;
      case "raceSelectName":
        setRace(selected);
        handleContentChange(type, { race: selected.value }, ctType);
        break;
    }
  };
  ...
  const handlePlaceStateChange = (options) => {
    if (
      ...
      setPlaceState([]);
      handleContentChange(type, { geography: [] }, ctType);
    } else {
      setPlaceState(options);
      handleContentChange(
        type,
        {
          geography: options.map((option) => option.value),
        },
        ctType
      );
    }
