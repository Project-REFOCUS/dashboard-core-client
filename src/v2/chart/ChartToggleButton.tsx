...
interface Props {
    handleOnChange: (value : GraphTypeEnum) => void;
    selected : GraphTypeEnum;
    options: GraphTypeEnum[];
}

function ChartToggleButton({handleOnChange, selected, options=[]}: Props) {

    const [ onHover, setOnHover ] = useState<boolean>(false);
    const [ mouseRef, setMouseRef ] = useState<HTMLElement | null>(null);
    const [ hoverText, setHoverText ] = useState<string | null>(null);

    const handleMouseOver = (event: React.MouseEvent<HTMLElement>, text: string) => {
    ...
    const toggleOnChange = (value : GraphTypeEnum) => {
        if(value !== null){
