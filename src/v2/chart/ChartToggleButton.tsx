...
function ChartToggleButton({handleOnChange, selected, options=[]}: Props) {
    ...
    return (
        ...
        >
            <ToggleButton value={GraphTypeEnum.LINE} sx={buttonSX} disabled={!options?.includes(GraphTypeEnum.LINE)} className={ !options?.includes(GraphTypeEnum.LINE) ? 'disabled-cursor' : '' }>
            ...
            <ToggleButton  value={GraphTypeEnum.BAR} sx={buttonSX} disabled={!options?.includes(GraphTypeEnum.BAR)} className={ !options?.includes(GraphTypeEnum.BAR) ? 'disabled-cursor' : '' }>
                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
